...
const getByLeague = promisify((opts, cb) => {
...
const getByTeams = promisify((opts, cb) => {
  const url = 'http://games.espn.com/ffl/schedule?' + [
    `leagueId=${opts.leagueId}`,
    `seasonId=${opts.seasonId}`
  ].join('&')
...
const parseHTMLByLeague = function(html) {
  ...
  $('.games-fullcol > table.tableBody tr:not(.tableSubHead)').each(function(index, element) {
    ...
    const away_path = $(this).children().eq(0).find('a').attr('href')
    const home_path = $(this).children().eq(3).find('a').attr('href')

    const away_id = parseInt(/teamId=(\d+)/ig.exec(away_path)[1], 10)
    const home_id = parseInt(/teamId=(\d+)/ig.exec(home_path)[1], 10)

    result[week].push({
      away_team: $(this).children().eq(0).find('a').text(),
      away_href: 'http://games.espn.com' + away_path,
      away_id: away_id,
      away_owners: $(this).children().eq(1).text().split(','),
      home_team: $(this).children().eq(3).find('a').text(),
      home_href: 'http://games.espn.com' + home_path,
      home_id: home_id,
