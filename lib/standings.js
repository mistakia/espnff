...
const getByOwner = promisify((opts, cb) => {
  const url = 'http://games.espn.com/ffl/standings?' + [
    `leagueId=${opts.leagueId}`,
    `seasonId=${opts.seasonId}`
  ].join('&')
  ...
  request({
    ...
    let items = parseHTMLByOwner(html)
...
const parseHTML = function(html) {
  ...
  $('table tr.sortableRow').each(function(index, element) {
    ...
    const path = $(this).children().eq(0).find('a').attr('href')
    const team_id = parseInt(/teamId=(\d+)/ig.exec(path)[1], 10)

    items.push({
      team: $(this).children().eq(0).find('a').text(),
      team_id: team_id,
...
const parseHTMLByOwner = function(html) {
  let result = {}
  let items = []
